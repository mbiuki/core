...
public class MultiPartSecurityRequestWrapper extends HttpServletRequestWrapper {
    ...
    boolean shouldCacheToDisk(final HttpServletRequest request) {
        ...
        return contentLength<=0 || contentLength > cacheToDiskSize ;
        
    }
    
    
    public MultiPartSecurityRequestWrapper(final HttpServletRequest request, final long cacheToDiskSize) throws IOException {
        super(request);
        this.cacheToDiskSize=cacheToDiskSize;
    ...
    private void testString(String lineToTest) {
        ...
        if (null == fileName 
                        ...
                        || fileName.indexOf("\\") != -1 
                        || fileName.indexOf("..") != -1
                        || !FileUtil.sanitizeFileName(fileName).equals(fileName)
